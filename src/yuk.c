...
{ /* this runs every time a source line is encountered */
  ...
  { /* debug */
	...
	case '@':
	  ...
	  puts("*\tview the GNU General Public License");
	  ...
	  puts("Roman numerals.");
#ifdef __DJGPP__
	  puts("You can press <CTRL>-<BREAK> to interrupt an executing program.");
#else
	  puts("You can press <CTRL>-C to interrupt an executing program.");
#endif
	  break;
	case 'q':
	  exit(0);
	...
	case 'h':
	  ...
	  break;
	case '*':
	  tempcharp=findandtestopen("COPYING.txt",globalargv[1],
				    "r",globalargv[2]);
	  sprintf(copyloc,"more < %s",tempcharp);
	  system(copyloc); /* display the GNU GPL copyright */
	  break;
	default:
