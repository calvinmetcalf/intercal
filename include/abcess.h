...
struct tag_ickthread
{
  ...
  unsigned skipto; /* compucome line number */
  jmp_buf cjb; /* keeps track of compucomes */
  ickthread* dsi; /* which thread's varforget and sb to use */
  ickthread* usesthis; /* for garbage collection purposes */
};
...
#define NEXTTHREAD if(printflow) printf("[%d:%lx]",lineno,(unsigned long)\
                                                          ickmt_cur); \
                                 if(setjmp(cjb) == 0) \
...
#if (MULTITHREAD != 0)
extern overop* oo_onespots;
extern overop* oo_twospots;
#endif
#endif
